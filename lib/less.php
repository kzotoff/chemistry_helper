...
// USAGE: lib/less.php?path=path/to/less.less
// path tp LESS file must be relative to this file
// font links at the LESS file must be relative to this file

// ALL CALLS ARE AT THE END
...
class lessc {
	...
	protected function lib_length($value) {
		if ($value[0] == "list") {
			return count($value[2]);
		}
		$this->throwError("array required");
	}
...
$lc = new lessc($_GET["path"]);

try{
	header('Content-Type: text/css');
	echo $lc->parse();
} catch (exception $ex){
	echo 'LESSC ERROR: '.$ex->getMessage();
}
